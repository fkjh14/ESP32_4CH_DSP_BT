...
size_t readsize = 0;
int blocksize = 256;  // Smaller blocksize for lower latency and better audio sync
...
//fifo
int data32b[128], data32b2[128], data32b3[128];  // Smart RAM optimization
#define fifomax 4096*2  // Further reduced buffer size for lower latency and less pitch issues
...
static void aux_i2s_task_handler(void *arg) {
	...
	while (1) {
		...
		if (IsBTConnected()==1 && lastbtstate==0) {			
			...
			int fill_count = 0;
			while (entries() < 4000 && fill_count < 50) {  // Reduced pre-fill for faster startup
		...
		if (IsBTConnected()==0)  {
		...
		else if (IsBTConnected()==1 && entries()<128) {  // Earlier refill trigger for smoother playback
			critical++;
			lastbtstate=1;
			int refill_count = 0;
			while (entries() < 2000 && refill_count < 25) {  // Smaller refill buffer for lower latency
				//i2s_write(0, emptybuffer, blocksize, &bytes_written, portMAX_DELAY);
				//i2s_write(1, emptybuffer, blocksize, &bytes_written, portMAX_DELAY);
				i2s_write(0, &data32b[126], 8, &bytes_written, portMAX_DELAY);
				i2s_write(1, &data32b2[126], 8, &bytes_written, portMAX_DELAY);
				refill_count++;
				if (refill_count % 10 == 0) {
					esp_task_wdt_reset();  // Reset watchdog
					vTaskDelay(1 / portTICK_PERIOD_MS);
				}
			}
			...
			for (int i=0; i<64; i++) {  // Process smaller chunks for lower latency
			...
			rptrtotal += 64;  // Only processed 64 samples
...
void aux_i2s_task_start_up(void) {
	...
	ESP_LOGI("HEAP", "Free heap before allocation: %d bytes", esp_get_free_heap_size());
	btbuffer = (short*) malloc(fifomax*sizeof(short));
	if (btbuffer == NULL) {
        ESP_LOGE("MALLOC", "Failed to allocate memory for btbuffer");
        ESP_LOGE("HEAP", "Free heap: %d bytes", esp_get_free_heap_size());
        return;
    }
    ESP_LOGI("MALLOC", "Successfully allocated %d bytes for btbuffer", fifomax*sizeof(short));
    ESP_LOGI("HEAP", "Free heap after allocation: %d bytes", esp_get_free_heap_size());
	//startup Aux Task
	if (s_aux_i2s_task_handle==NULL) { 
    xTaskCreate(aux_i2s_task_handler, "AuxI2ST", 4096, NULL, configMAX_PRIORITIES - 1, &s_aux_i2s_task_handle);
