...
void bt_app_rc_ct_cb(esp_avrc_ct_cb_event_t event, esp_avrc_ct_cb_param_t *param)
{
    ...
    default:
        ESP_LOGW(BT_RC_CT_TAG, "Unhandled AVRC CT event: %d", event);
        break;
    }
}
...
void bt_app_rc_tg_cb(esp_avrc_tg_cb_event_t event, esp_avrc_tg_cb_param_t *param)
{
    ...
    case ESP_AVRC_TG_REGISTER_NOTIFICATION_EVT:
        ...
        break;
    default:
        ESP_LOGW(BT_RC_TG_TAG, "Unhandled AVRC TG event: %d", event);
        break;
    }
}
...
static void bt_av_hdl_a2d_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_A2D_AUDIO_CFG_EVT: {
        ...
        if (a2d->audio_cfg.mcc.type == ESP_A2D_MCT_SBC) {
            int sample_rate = 44100;  // Default to 44100 Hz instead of 16000
            ...
            if (sample_rate != 48000 && sample_rate != 44100) esp_restart();
            // i2s_adjust_pll_x64(sample_rate);

            initDSPFilters((float)sample_rate);
...
void bt_av_notify_evt_handler(uint8_t event_id, esp_avrc_rn_param_t *event_parameter)
{
    ...
    case ESP_AVRC_RN_PLAY_POS_CHANGED:
        ...
        break;
    default:
        ESP_LOGW(BT_AV_TAG, "Unhandled AVRC notification event: %d", event_id);
        break;
    }
}
...
static void bt_av_hdl_avrc_tg_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_AVRC_TG_CONNECTION_STATE_EVT: {
        ...
        if (rc->conn_stat.connected) {
            ...
            SetBTConnected(1);
            ESP_LOGI(BT_RC_TG_TAG, "Connected");
            //SwitchAuxToBT();
           
        } else {
        	SetBTConnected(0);
            //SwitchBTToAux();
            ESP_LOGI(BT_RC_TG_TAG, "BT Disconnected");
